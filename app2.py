...
def feature_test(datasample):
	datasample['Hipx_baseline'] = (datasample['leftHip_x'] + datasample['rightHip_x'])/ 2
	datasample['Hipy_baseline'] = (datasample['leftHip_y'] + datasample['rightHip_y'])/ 2

	# wrist hip distance
	x = pow((datasample['leftWrist_x'] - datasample['Hipx_baseline']), 2)
	y = pow((datasample['leftWrist_y'] - datasample['Hipy_baseline']), 2)
	datasample['leftWrist_hip_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	x = pow((datasample['rightWrist_x'] - datasample['Hipx_baseline']), 2)
	y = pow((datasample['rightWrist_y'] - datasample['Hipy_baseline']), 2)
	datasample['rightWrist_hip_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	# wrist nose distance
	x = pow((datasample['leftWrist_x'] - datasample['nose_x']), 2)
	y = pow((datasample['leftWrist_y'] - datasample['nose_y']), 2)
	datasample['leftWrist_nose_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	x = pow((datasample['rightWrist_x'] - datasample['nose_x']), 2)
	y = pow((datasample['rightWrist_y'] - datasample['nose_y']), 2)
	datasample['rightWrist_nose_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	# Elbow hip distance
	x = pow((datasample['leftElbow_x'] - datasample['Hipx_baseline']), 2)
	y = pow((datasample['leftElbow_y'] - datasample['Hipy_baseline']), 2)
	datasample['leftElbow_hip_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	x = pow((datasample['rightElbow_x'] - datasample['Hipx_baseline']), 2)
	y = pow((datasample['rightElbow_y'] - datasample['Hipy_baseline']), 2)
	datasample['rightElbow_hip_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	# Elbow nose distance
	x = pow((datasample['leftElbow_x'] - datasample['nose_x']), 2)
	y = pow((datasample['leftElbow_y'] - datasample['nose_y']), 2)
	datasample['leftElbow_nose_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	x = pow((datasample['rightElbow_x'] - datasample['nose_x']), 2)
	y = pow((datasample['rightElbow_y'] - datasample['nose_y']), 2)
	datasample['rightElbow_nose_distance']= pd.Series(np.round(np.sqrt(x+y),3))

	startframe = 0
	endframe = datasample.shape[0]
	...
	data_selected_for_featurecreation = datasample.loc[rowlist, collist].copy()
	# feature_step = pd.Series(np.round(np.mean(data_selected_for_featurecreation, axis=0), 3))
	feature_step = np.round(np.mean(data_selected_for_featurecreation.to_numpy(), axis=0), 3)
	print(feature_step.shape)
	feature_sample = feature_step.reshape(1, -1)
